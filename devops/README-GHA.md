...
2. Go to your repository at [collective/plone-edu](https://github.com/collective/plone-edu).
3. Click on `Settings` at the top-right corner.
4. In the left sidebar, click on `Environments`.
5. Press `New environment`.
6. Name the environment `plone-edu.example.com` and hit `Configure environment`.
...
|-------------|--------------|-------------|
| DEPLOY_HOST | plone-edu.example.com | The hostname or IP address of your Docker Swarm manager. |
...
| Name     | Value |
|----------|-------|
| LIVE_ENV | plone-edu.example.com |
...
- [Backend Tests Workflow](https://github.com/collective/plone-edu/actions/workflows/backend.yml)
- [Frontend Tests Workflow](https://github.com/collective/plone-edu/actions/workflows/frontend.yml)

Upon successful completion of the tests, Docker images for the Backend (`ghcr.io/collective/plone-edu-backend`) and Frontend (`ghcr.io/collective/plone-edu-frontend`) will be available.

### Initiating Manual Deployment

1. Navigate to [Manual Deployment of plone-edu.example.com](https://github.com/collective/plone-edu/actions/workflows/manual-deploy.yml).
...
- Initiate a new deployment using the `devops/stacks/plone-edu.example.com.yml` stack.
