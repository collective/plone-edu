...
PROJECT_NAME=plone-edu
STACK_NAME=plone-edu-example-com
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://plone-edu.localhost"
...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t collective/plone-edu-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t collective/plone-edu-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name plone-edu-backend-acceptance -d collective/plone-edu-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name plone-edu-frontend-acceptance --link plone-edu-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d collective/plone-edu-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop plone-edu-frontend-acceptance
	@docker stop plone-edu-backend-acceptance
